            ...
            <tbody>
              ...
              <tr>
                ...
                <td class="text-center">
                  <div class="btn-group">
                    <button type="button" class="btn btn-sm btn-info" onclick="ClearInput({{ $baris->kd }}, '{{ $baris->nik }}', '{{ $baris->nama }}', '{{ $baris->posisi }}', true)"><i class="icon-pencil3"></i></button>
                ...
                <div class="col-sm-6">
                  <div class="form-group">
                    ...
                    <select id="posisi" name="posisi" class="form-control">
                      <option value="Purchasing" {{ old('posisi')=='Purchasing' ? 'selected': '' }}>Purchasing</option>
                      <option value="Direktur" {{ old('posisi')=='Direktur' ? 'selected': '' }}>Direktur</option>
                      <option value="Administrator" {{ old('posisi')=='Administrator' ? 'selected': '' }}>Administrator</option>
                    </select>
                  </div>
                </div>
                <div class="col-sm-6">
...
<script>
  ...
  function ClearInput(kd, nik, nama, posisi, show)
  {
    if(show){
      $('#card-akun').show('slow');
  ...
  function ReloadTampil(data, op){
    data['tombol'] = '<div class="btn-group">'+
      '<button type="button" class="btn btn-sm btn-info" onclick="ClearInput('+data['kd']+', \''+data['nik']+'\', \''+data['nama']+'\', \''+data['posisi']+'\', true)"><i class="icon-pencil3"></i></button>'+
